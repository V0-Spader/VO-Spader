...
//Create a markup for each episode on a given list and to dom
...
function populateEpisodes(episodes) {
  ...
  for (let episode of episodes) {
    ...
    $episodeList.aappend($item);
  }
  $episodeList.show();
}

async function getEpisodeAndDisplay(event) {
  const showId = $(event.target).closest(".Show").data("show-id");
  const episodes = await getEpisodeOfShow(showId);
