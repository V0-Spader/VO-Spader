...
import { create, findAll, get, update, remove } from "./company.js";
...
describe("create", function () {
  ...
  test("works", async function () {
    let company = await create(newCompany);
...
describe("findAll", function () {
  ...
  test("works: by min employees", async function () {
    let companies = await findAll({ minEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("works: by max employees", async function () {
    let companies = await findAll({ maxEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: by name", async function () {
    let companies = await findAll({ name: "1" });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: empty list on nothing found", async function () {
    let companies = await findAll({ name: "nope" });
    expect(companies).toEqual([]);
  });

  test("bad request if invalid min > max", async function () {
    try {
      await findAll({ minEmployees: 10, maxEmployees: 1 });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
...
describe("update", function () {
  ...
  test("works", async function () {
    let company = await update("c1", updateData);
    expect(company).toEqual({
      handle: "c1",
      ...updateData,
    });
  ...
  test("works: null fields", async function () {
    ...
    let company = await update("c1", updateDataSetNulls);
    expect(company).toEqual({
      handle: "c1",
      ...updateDataSetNulls,
    });
