...
describe("POST /auth/register", function() {
  test("should allow a user to register in", async function() {
    ...
    let { username, admin } = verify(response.body.token, SECRET_KEY);
...
describe("DELETE /users/[username]", function() {
  ...
  test("should delete an existing user and return {message: 'deleted'}", async () => {
    const response = await request(app)
      .delete('/existingUser') // Adjust username
      .expect(200);
  ...
  test("should return 404 if user doesn't exist", async () => {
    const response = await request(app)
      .delete('/nonExistingUser') // adjust the username
      .expect(404);
